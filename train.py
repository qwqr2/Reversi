    ...
    def run(self):
        ...
        try:
            for i in range(self.game_batch_num):
                print(f"开始收集第 {i+1}/{self.game_batch_num} 批自我对弈数据...")
                ...
                if (i+1) % self.check_freq == 0:
                    ...
                    print(f"模型已保存到 './current_policy.model'")
                    if win_ratio > self.best_win_ratio:
                        print("New best policy!!!!!!!!")
                        self.best_win_ratio = win_ratio
                        # 更新最佳策略
                        self.policy_value_net.save_model('./best_policy.model')
                        print(f"最佳模型已保存到 './best_policy.model'")
                        # 修复变量名称错误
                        if (self.best_win_ratio == 1.0 and self.AIPlayer_num < 500):
                            self.AIPlayer_num += 100
                            print(f"提升纯MCTS对手强度至 {self.AIPlayer_num} 次模拟")
                            self.best_win_ratio = 0.0
